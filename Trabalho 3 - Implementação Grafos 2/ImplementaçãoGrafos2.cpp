...
void buscaProfundidade(int inicio)
{
    ...
    while (!pilha.empty())
    {
        ...
        {
            visitado[atual] = true;
            cout << "Visitando: " << atual << " (" << nomesVertices[atual] << ")\n";

            for (int i = 0; i < adjacencias[atual].size(); i++)
            {
                if (!visitado[adjacencias[atual][i].vertice])
                {
                    pilha.push_back(adjacencias[atual][i].vertice);
                }
            }
        }
    }
...
void buscaLargura(int inicio)
{
    ...
    while (!fila.empty())
    {
        ...
        for (int i = 0; i < adjacencias[atual].size(); i++)
        {
            if (!visitado[adjacencias[atual][i].vertice])
            {
                visitado[adjacencias[atual][i].vertice] = true;
                fila.push_back(adjacencias[atual][i].vertice);
            }
        }
    }
    cout << "================================\n";
